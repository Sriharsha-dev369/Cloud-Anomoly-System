...
import { AwsCredentials, createCloudWatchClient, createEc2Client } from '../utils/awsClientFactory';
...
// Builds a CloudAdapter scoped to the provided SDK clients.
// Both the global singleton and per-user adapters use this factory.
function buildAwsAdapter(cwClient: CloudWatchClient, ec2Client: EC2Client): CloudAdapter {
  ...
  return {
    async getMetrics(resourceId: string, resource: Resource, since?: string): Promise<Metric[]> {
      ...
      try {
        ...
        const response = await withRetry(() => cwClient.send(cmd));
    ...
    async stopResource(resourceId: string): Promise<void> {
      ...
      try {
        await withRetry(() => ec2Client.send(new StopInstancesCommand({ InstanceIds: [instanceId] })));
    ...
    async startResource(resourceId: string): Promise<void> {
      ...
      if (!instanceId) return;
      try {
        await withRetry(() => ec2Client.send(new StartInstancesCommand({ InstanceIds: [instanceId] })));
...
// Global singleton — uses env-level credentials (demo/global AWS mode).
export const awsAdapter = buildAwsAdapter(
  new CloudWatchClient({ region: AWS_REGION }),
  new EC2Client({ region: AWS_REGION }),
);

// Per-user adapter factory — uses the user's stored credentials.
export function createUserAwsAdapter(creds: AwsCredentials): CloudAdapter {
  return buildAwsAdapter(createCloudWatchClient(creds), createEc2Client(creds));
}
